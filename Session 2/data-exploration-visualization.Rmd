...
Remove those are highly correlated
```{r}
to_remove = c('int_rate', 'funded_amnt', 'funded_amnt_inv', 'installment', 'out_prncp_inv', 'total_pymnt_inv', 'total_rec_prncp')
full_df<-full_df[ !(names(full_df) %in% to_remove)] 
```
...
```{r}
# 
table(full_df$issue_d) # can see that we have data from April 2013 to September 2015
# transform the issue_d
full_df$issue_d<-dmy(paste0("01-", full_df$issue_d))
g5<-ggplot(full_df, aes(x=issue_d, y=loan_amnt)) + stat_summary(fun.y = "sum", geom = "bar", fill="ivory") + xlab("Issued Date") + ylab("Loan Amount") + ggtitle("Loan amount over the Issued Dates")
g5
```

Visualize the states - loan amount
```{r}
library(maps)
...
# If you also want to save the image as png
